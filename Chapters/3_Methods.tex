...
\Require \textit{FPGA}: DE10-Lite board (or similar target)
\Require \textit{Quartus Prime}: Intel FPGA development software
...
\State Build netlist structure: $\mathcal{G} = (V, E)$ where $V$ = gates, $E$ = wires

\Statex
\Phase{Phase 3: HDL Generation}
\For{each layer $\ell$ in model}
    \State Declare intermediate wires/signals
    \For{each gate $g$ in layer $\ell$}
        \State Map $g$ to HDL assignment (Verilog \texttt{assign} or VHDL signal)
    \EndFor
\EndFor
\State Insert NOT gates for negated inputs

\Statex
\Phase{Phase 4: Synthesis}
\State Create Quartus project for target FPGA
\State Import HDL files and assign I/O pins
\State Run synthesis with timing constraint (e.g., 50\,MHz)
\State Verify LUT utilization and timing closure

\Statex
\Phase{Phase 5: Deployment}
\State \textbf{Optional:} Simulate in ModelSim for functional verification
...
\Require \textit{Dataset}: Labeled data with $N$ classes
\Require \textit{sf}: Switching frequency threshold $\in [0, 1]$
...
\For{each class $i \in \{1, \ldots, N\}$}
    \State Register hooks to count node activations $a_j^{(i)}$ for each node $j$
    \State Forward pass all samples in $D_i$
    \State $A^{(i)} \gets$ activation counts per node
\EndFor
...
A lightweight backend extracts the gate-level configuration from a trained DiffLogic model (commonly stored as a \texttt{.pth} file) and generates a structured graph representation in JSON. This graph is then rendered in a web-based frontend using ReactFlow, a React library for building interactive node-based graphs. Users can zoom, pan, and explore various sections of the DAG, allowing granular exploration of how specific gates and their connections contribute to the final prediction. A technical workflow of the tool is described as follows:
...
\State Load model weights and architecture
\State Extract nodes $V = \{v_i : \text{gate type, probability}\}$
\State Extract edges $E = \{(v_i, v_j) : \text{signal connections}\}$

\Statex
\Phase{Phase 2: Graph Construction}
\State Build DAG $\mathcal{G} = (V, E)$ from extracted components
\State Assign layer positions for hierarchical layout

\Statex
\Phase{Phase 3: JSON Serialization}
\State Format $\mathcal{G}$ as JSON for frontend consumption
\State Include gate metadata (type, activation probability)

\Statex
\Phase{Phase 4: Visualization Rendering}
\State Render DAG using ReactFlow graph library
\State Enable zoom, pan, and node selection interactions

\Statex
\Phase{Phase 5: Interactive Exploration}
\State Highlight activated gates based on input image
\State Display node details on click (gate type, connections)
